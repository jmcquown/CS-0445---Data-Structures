...
*/
	...
	public void buildInorder(String fileName) {
		try {
			...
			int numOfNodes = IS.readInt();	//Read the number of nodes (First line of the file)
			
			
			root = RecReadTree(IS, numOfNodes);
			IS.close();
	...
	//Recursive method that reads a .dat file and builds a binary tree from the .data file data
	public BinaryNode<T> RecReadTree(ObjectInputStream IS, int nodes) {
		...
		if (nodes == 0)
			...
			int leftNodes = (nodes - 1) / 2 + (nodes - 1) % 2;
			int rightNodes = (nodes - 1) / 2;
					
			node.setLeftChild(RecReadTree(IS, leftNodes));
			...
			node.setRightChild(RecReadTree(IS, rightNodes));
		}
